...
{
    // Check prefix
    if(uri.scheme() != QString("rubycoin"))
        return false;
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Rubycoin.lnk";
}
...
{
    return GetAutostartDir() / "rubycoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Rubycoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  rubycoin-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetRedThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(72,0,0); color: rgb(255,255,255); }"
                      ...
                      "QTextEdit      { background: rgb(37,0,0); color: rgb(255,255,255); }"
                      ...
                      "QTabBar::tab   { color: rgb(120,127,139); border: 1px solid rgb(126,0,0); border-bottom: none; padding: 5px; }"
